...
class ImageNetIDDataset(torch.utils.data.Dataset):
    def __init__(self, root_dirname, phase):
...
class TinyImageNetIDDataset(torch.utils.data.Dataset):
    def __init__(self, data_root, phase):
        ...
        if self.phase == 'train':
            ...
            self.file_name_to_category_id = OrderedDict()
    ...
    def __getitem__(self, item):
        if self.phase == "train":
            img_id, (img, label) = self.images_id[item], self.dataset[item]
        else:
...
class DataLoaderMaker(object):
    ...
    @staticmethod
    def get_imgid_img_label_data_loader(datasetname, batch_size, is_train, shuffle):
        """
        ...
        if datasetname == 'ImageNet':
            loader = torch.utils.data.DataLoader(ImageNetIDDataset(IMAGE_DATA_ROOT[datasetname], phase),
                                                 batch_size=batch_size, num_workers=0, shuffle=shuffle)

        elif datasetname == "CIFAR-10":
            loader = torch.utils.data.DataLoader(CIFAR10IDDataset(IMAGE_DATA_ROOT[datasetname], phase),
                                                 batch_size=batch_size, num_workers=0, shuffle=shuffle)
        elif datasetname == "CIFAR-100":
            loader = torch.utils.data.DataLoader(CIFAR100IDDataset(IMAGE_DATA_ROOT[datasetname], phase),
                                                 batch_size=batch_size, num_workers=0, shuffle=shuffle)
        elif datasetname == "MNIST":
    ...
    def get_candidate_attacked_data(datasetname, batch_size):
        dataset = NpzExtraDataset(datasetname)
        loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, num_workers=0, shuffle=False)
        return loader
