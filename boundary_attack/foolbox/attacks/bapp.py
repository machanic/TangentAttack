...
class BoundaryAttackPlusPlus(Attack):
    ...
    * ability to specify the batch size
    ...
    def attack(
        ...
        # to draw distance-v.s.-query plots
        self.stats_query_counts \
            = np.zeros((iterations,), dtype=np.int)
        self.stats_distances \
            = np.zeros((iterations,), dtype=np.float64)
        self.stats_all_steps = [None for _ in range(iterations)]
        ...
        for step in range(1, iterations + 1):
            ...
            self.stats_query_counts[step - 1] = a._total_prediction_calls
    ...
    def initialize_starting_point(self, a):
        # we modify the original bapp.py, such that it will apply binary search even if starting_point is provided
        ...
        if starting_point is not None:
            ...
            while True:
                starting_point = np.random.uniform(self.clip_min, self.clip_max, size=self.shape)
                _, success = a.forward_one(starting_point.astype(self.external_dtype))
                num_evals += 1
                if success:
                    break
                if num_evals > 1e4:
        ...
        while high - low > 0.001:
            mid = (high + low) / 2.0
            blended = (1 - mid) * a.unperturbed + mid * starting_point
