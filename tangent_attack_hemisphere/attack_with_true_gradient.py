...
class TangentAttack(object):
    def __init__(self, model, dataset,  clip_min, clip_max, height, width, channels, norm, epsilon, iterations=40, gamma=1.0,
                 ...
                 max_num_evals=1e4, init_num_evals=100, maximum_queries=10000,true_gradient=False):
        """
        ...
        self.approx_true_grad_cos_similarity = defaultdict(OrderedDict)
        self.approx_true_grad_distance = defaultdict(OrderedDict)
    ...
    # def soft_argmax(self, input_tensor):
    #     assert input_tensor.dim() == 2
    #     softmax = torch.nn.Softmax(1)
    #     return torch.matmul(softmax(input_tensor.float()), torch.arange(input_tensor.size(1)).float().to(input_tensor.device)).ceil()
    #
    # def decision_loss(self, logits, true_labels, target_labels):
    #     softmax_val  = self.soft_argmax(logits)
    #     argmax_val = logits.max(1)[1]
    #     if target_labels is None:
    #         return argmax_val != true_labels
    #     else:
    #         return argmax_val == target_labels
    ...
    def get_grad(self, model, x, true_labels, target_labels):
        ...
        with torch.enable_grad():
            ...
            logits = torch.softmax(self.model(x),dim=1)
            if true_labels is not None:
                true_labels = true_labels.cuda()
            if target_labels is not None:
                target_labels = target_labels.cuda()
            loss = self.cw_loss(logits, true_labels, target_labels)
            gradient = torch.autograd.grad(loss, x, retain_graph=False)[0]
            gradient /= torch.norm(gradient,p=2)
    ...
    def attack(self, batch_index, images, target_images, true_labels, target_labels):
        ...
        for j in range(self.num_iterations):
            ...
            if self.true_gradient:
                true_gradf = self.get_grad(self.model, perturbed, true_labels, target_labels)
                gradf = self.approximate_gradient(perturbed, true_labels, target_labels, num_evals, 0.01)
                cos_theta = torch.dot(true_gradf.view(-1), gradf.view(-1)).item() / (torch.norm(true_gradf.view(-1)).item()
                                                                                     * torch.norm(gradf.view(-1)).item())
                log.info("{}-th image, {}-th iteration, cos theta is {:.5f}, diff mean is {:.4f}".format(batch_index+1, j+1,
                                                                    cos_theta, torch.abs(true_gradf - gradf).max().item()))
                for inside_batch_index, index_over_all_images in enumerate(batch_image_positions):
                    self.approx_true_grad_cos_similarity[index_over_all_images][query[inside_batch_index].item()] = cos_theta
                    self.approx_true_grad_distance[index_over_all_images][query[inside_batch_index].item()] = torch.abs(true_gradf - gradf).mean().item()
    ...
    def attack_all_images(self, args, arch_name,  result_dump_path):
        ...
        meta_info_dict = {"avg_correct": self.correct_all.mean().item(),
                          ...
                          "approx_and_true_grad_cosine_similarity":self.approx_true_grad_cos_similarity,
                          "approx_and_true_grad_avg_distance":self.approx_true_grad_distance,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--true-gradient',action='store_true')
    ...
    if args.all_archs:
        ...
        if args.attack_defense:
            ...
            assert args.true_gradient
            log_file_path = osp.join(args.exp_dir, "run_{}_true_gradients.log".format(args.arch))
        else:
    ...
    for arch in archs:
        if args.attack_defense:
            ...
            if args.true_gradient:
                save_result_path = args.exp_dir + "/{}_replace_with_true_gradient_result.json".format(arch)
        else:
        ...
        attacker = TangentAttack(model, args.dataset, 0, 1.0, model.input_size[-2], model.input_size[-1], IN_CHANNELS[args.dataset],
                                     ...
                                     max_num_evals=1e4,
                                     init_num_evals=args.init_num_eval_grad, maximum_queries=args.max_queries,true_gradient=args.true_gradient
                                     )
