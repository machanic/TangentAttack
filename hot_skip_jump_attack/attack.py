...
from config import CLASS_NUM, MODELS_TEST_STANDARD, PY_ROOT, IN_CHANNELS
...
class HotSkipJumpAttack(object):
    def __init__(self, model, dataset,  clip_min, clip_max, height, width, channels, norm, epsilon, iterations=40, gamma=1.0, stepsize_search='geometric_progression',
        ...
        assert len(self.label_data_index_dict) == CLASS_NUM[self.dataset_name], "label data dict length is {}".format(len(self.label_data_index_dict))
            ...
            while True:
                ...
                if num_eval > 1000:
                    ...
                    if self.dataset_name == "ImageNet":
                        initialization = self.get_image_of_class_ImageNet(target_labels, self.dataset, self.model).squeeze()
                    else:
                        initialization = self.get_image_of_class(target_labels, self.dataset, self.model).squeeze()
    ...
    def binary_search_batch(self, original_image, perturbed_images, true_labels, target_labels):
        ...
        return out_image, dist, num_eval
    ...
    def approximate_gradient(self, sample, true_labels, target_labels, num_evals, delta):
        ...
        # query the model.
        if self.dataset_name=="ImageNet":  # FIXME save GPU memory
            if perturbed.size(0) >= 4:
    ...
    def attack(self, batch_index, images, target_images, true_labels, target_labels):
        ...
        for j in range(self.num_iterations):
            ...
            if self.stepsize_search == 'geometric_progression':
                # find step size.
                epsilon = self.geometric_progression_for_stepsize(perturbed, true_labels, target_labels, update, dist, cur_iter)
                query += 1
                # Update the sample.
    ...
    def attack_all_images(self, args, arch_name, target_model, result_dump_path):
        ...
        for batch_index, (images, true_labels) in enumerate(self.dataset_loader):
            ...
            if args.targeted:
                ...
                if self.dataset_name == "ImageNet":
                    target_images = self.get_image_of_class_ImageNet(target_labels, self.dataset, target_model)
                else:
                    target_images = self.get_image_of_class(target_labels, self.dataset, target_model)
            else:
                target_labels = None
                target_images = None
            ...
            adv_pred = adv_logit.argmax(dim=1).detach().cpu()
            ...
            success = (1-not_done) * correct.detach().cpu()
