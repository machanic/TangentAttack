...
class SamplingProvider:
    ...
    def __init__(self, shape, n_threads=1, queue_lengths=40):
        self.shape = shape  # (channels, image_height, image_width)
        self.n_threads = n_threads
        self.queue_lengths = queue_lengths
        self.queue_normal = deque()
        self.queue_perlin = deque()

        self.perlin_fade = calc_fade(shape[1], freq=20)             # Tiny ImageNet: 5 works well. ImageNet: 20 is better.
    ...
    def _thread_fun(self, thread_id):
        ...
        while getattr(t, 'do_run', True):
            ...
            if rnd_perlin is None:
                rnd_perlin = create_perlin_noise(px=self.shape[1], color=self.perlin_color, batch_size=1,
