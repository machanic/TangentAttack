...
def draw_query_distortion_figure(dataset, norm, targeted, arch, fig_type, dump_file_path, xlabel, ylabel):
    ...
    if dataset=="ImageNet":
        ...
        plt.yticks([0.1,1, max_y/2, max_y+0.1], fontsize=45)
...
if __name__ == "__main__":
    args = parse_args()
    dump_folder = "/home1/machen/hard_label_attacks/paper_chinese_figures/"
    os.makedirs(dump_folder, exist_ok=True)

    if "CIFAR" in args.dataset:
        archs = ["pyramidnet272","gdas","WRN-28-10-drop","WRN-40-10-drop"]
    else:
        archs = ["resnext101_64x4d","inceptionv4","senet154","resnet101","inceptionv3","resnet101"]
    for model in archs:
