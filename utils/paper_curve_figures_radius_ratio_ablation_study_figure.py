...
method_name_to_paper = {"tangent_attack":"TA (hemisphere)",
                        "ellipsoid_tangent_attack":"G-TA (semi-ellipsoid)",
...
def draw_query_distortion_figure(dataset, norm, targeted, arch, fig_type, dump_file_path, xlabel, ylabel):
    ...
    for idx, ((dataset, norm, targeted, method, radius_ratio), (x,y)) in enumerate(data_info.items()):
        ...
        if np.min(y) < min_y:
            min_y = np.min(y)
        line, = plt.plot(x, y, label="radius ratio $r={}$".format(radius_ratio),
...
def draw_random_HSJA_TangentAttack_query_distortion_figure(dataset, norm, targeted, arch, fig_type, dump_file_path, xlabel, ylabel):
    ...
    colors =['b', 'g', 'c', 'y', 'k', 'peru', "gold",'aqua','fuchsia']
    # markers = [".",",","o","^","s","p","x"]
    # max_x = 0
    # min_x = 0
    our_method1 = 'TA (hemisphere)'
    our_method2 = 'G-TA (semi-ellipsoid)'
...
if __name__ == "__main__":
    args = parse_args()
    dump_folder = "/home1/machen/hard_label_attacks/ablation_study_figures/radius_ratio/"
