    ...
    for (dataset, norm, targeted, method), dir_path in dataset_path_dict.items():
        for file_path in os.listdir(dir_path):
            if arch in file_path and file_path.endswith(".json") and not file_path.startswith("tmp"):
                ...
                radius_schedule = ma.group(1)
                if radius_schedule == "log2_geometric_progression_radius":
                    continue
                ...
                x = np.array(x)
                y = np.array(y)
                data_info[(dataset, norm, targeted, radius_schedule)] = (x,y)
...
schedule_name_dict = {"binary_search_radius":"Binary Search Radius","fixed_radius":"Fixed Radius",
                      "geometric_progression_radius": "Our Approach"}
...
def draw_query_distortion_figure(dataset, norm, targeted, arch, fig_type, dump_file_path, xlabel, ylabel):
    ...
    max_y = 0
    min_y= 999
    for idx, ((dataset, norm, targeted, radius_schedule), (x,y)) in enumerate(data_info.items()):
        color = colors[idx]
        if radius_schedule == our_schedule:
            color = "r"
        x = np.asarray(x)
        y = np.asarray(y)
        if np.max(y) > max_y:
        ...
        if np.min(y) < min_y:
            min_y = np.min(y)
        line, = plt.plot(x, y, label=schedule_name_dict[radius_schedule], color=color, linestyle=linestyle_dict[linestyles[idx]], marker=markers[idx], markersize=20, linewidth=3.0)
    ...
    plt.legend(loc='upper right', prop={'size': 45},labelcolor='linecolor',
...
if __name__ == "__main__":
    args = parse_args()
    dump_folder = "/home1/machen/hard_label_attacks/paper_figures/radius_ablation_study/"
    os.makedirs(dump_folder, exist_ok=True)

    arch = "resnet101"
    file_path  = dump_folder + "{dataset}_{model}_{norm}_{target_str}_attack_radius_ablation_study.pdf".format(dataset=args.dataset,
