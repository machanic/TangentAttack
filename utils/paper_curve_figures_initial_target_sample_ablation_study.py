...
initial_target_sample_schedule_dict = {"random_initial_sample": "a random image", "best_initial_sample": "the minimum distortion",
                      "worst_initial_sample": "the maximum distortion"}
...
def draw_query_distortion_figure(dataset, norm, targeted, arch, fig_type, dump_file_path, xlabel, ylabel):
    ...
    for idx, ((dataset, norm, targeted, initial_schedule), (x,y)) in enumerate(data_info.items()):
        ...
        if np.min(y) < min_y:
            min_y = np.min(y)
        line, = plt.plot(x, y, label=initial_target_sample_schedule_dict[initial_schedule], color=color, linestyle=linestyle_dict[linestyles[idx]], marker=markers[idx], markersize=20, linewidth=3.0)
...
if __name__ == "__main__":
    args = parse_args()
    dump_folder = "/home1/machen/hard_label_attacks/paper_figures/initial_target_sample_ablation_study/"
