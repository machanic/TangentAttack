...
class UNet(nn.Module):
    def __init__(self, input_size=32, n_channels=3, bilinear=True,
                 normalization_type='none', base_width=16, init_std=1/32.):
        ...
        self.down2 = Down(widths[1], widths[2], normalization_type=normalization_type)
        factor = 2 if bilinear else 1
        self.down3 = Down(widths[2], widths[3] // factor, normalization_type=normalization_type)
        self.up1 = Up(widths[3], widths[2] // factor, bilinear, normalization_type=normalization_type)
        self.up2 = Up(widths[2], widths[1] // factor, bilinear, normalization_type=normalization_type)
        self.up3 = Up(widths[1], widths[0], bilinear, normalization_type=normalization_type)
        self.outc = OutConv(widths[0], n_channels)

        # mean / std output
        self.normal_mean = nn.Parameter(torch.zeros(n_channels, input_size, input_size))
        self.normal_logstd = nn.Parameter(torch.ones(1) * math.log(self.init_std))
        self.mean_shape = (n_channels, input_size, input_size)
    ...
    def forward(self, adv_image, image, label, target, output_fields):
        ...
        if 'grad' in output_fields:
            ...
            x3 = self.down2(x2)   # widths[2] channels
            x4 = self.down3(x3)   # widths[3] // 2 channels if bilinear, else widths[3]
            x = self.up1(x4, x3)  # widths[2] // 2 channels if bilinear, else widths[2]
            x = self.up2(x, x2)   # widths[1] // 2 channels if bilinear, else widths[1]
            x = self.up3(x, x1)   # widths[0] channels
