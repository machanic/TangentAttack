...
class HotSkipJumpAttack(object):
    def __init__(self, model, dataset,  clip_min, clip_max, height, width, channels, norm, epsilon,
        ...
        self.target_labels_all = torch.zeros_like(self.query_all).long()
    ...
    def attack_all_images(self, args, arch_name, result_dump_path):
        ...
        for batch_index, (images, true_labels) in enumerate(self.dataset_loader):
            ...
            for key in ["target_labels"]:
                ...
                value = eval(key)
                value_all[selected] = value.detach().cpu()

        log.info('{} is attacked finished ({} images)'.format(arch_name, self.total_images))
        log.info('Saving results to {}'.format(result_dump_path))
        target_labels_all = self.target_labels_all.detach().cpu().numpy().astype(np.int64)
        np.save(result_dump_path, target_labels_all)
        log.info("done, write labels info to {}".format(result_dump_path))


def get_exp_dir_name(dataset):
    return "./target_class_labels/{}".format(dataset)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp-dir', type=str, help='directory to save results and logs')
    ...
    if args.attack_defense and args.defense_model == "adv_train_on_ImageNet":
        args.max_queries = 20000
    args.exp_dir = get_exp_dir_name(args.dataset)
    ...
    print_args(args)
    for arch in archs:
        save_result_path = args.exp_dir + "/label.npy"
        if args.attack_defense:
